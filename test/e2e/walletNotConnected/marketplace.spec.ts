...
test(`filter by instance`, async ({ page }) => {
	...
	const allInstancesMatch = instancesOnPage.every((instance) =>
		instance?.endsWith(instanceToFilter)
	);
...
test('filter by architecture', async ({ page }) => {
	...
	const allarchitecturesMatch = architecturesOnPage.every((architecture) =>
		architecture?.endsWith(architectureToFilter)
	);
