...
import { DEFAULT_RECEIVER_STAKING_DATA, DEFAULT_WALLET_BALANCE } from '$lib/utils/constants/storeDefaults';
import { QUERY_TO_CHECK_IF_SIGNER_EXISTS, QUERY_TO_GET_MPOND_BALANCE, QUERY_TO_GET_MPOND_TO_POND_CONVERSION_HSTORY, QUERY_TO_GET_POND_AND_MPOND_BRIDGE_ALLOWANCES, QUERY_TO_GET_POND_BALANCE_QUERY, QUERY_TO_GET_POND_TO_MPOND_CONVERSION_HSTORY, QUERY_TO_GET_RECEIVER_POND_BALANCE, QUERY_TO_GET_RECEIVER_STAKING_DATA, QUERY_TO_MPOND_REQUESTED_FOR_CONVERSION } from '$lib/utils/constants/subgraphQueries';
...
function subgraphQueryWrapper(query, variables) {
    const options = {
        method: 'POST',
        body: JSON.stringify({
            query,
 ...
 */
export async function getPondBalance(address) {
    ...
    const options = subgraphQueryWrapper(query, queryVariables);
    try {
 ...
 */
export async function getMPondBalance(address) {
...
export async function getReceiverPondBalanceFromSubgraph(address) {
 ...
 */
export async function getReceiverStakingDataFromSubgraph(address) {
    ...
    try {
        ...
        if (params?.length === 2) {
            let startTime = params.find((param) => param.id === 'START_TIME').value;
            let epochLength = params.find((param) => param.id === 'EPOCH_LENGTH').value;
            //string to number
        ...
        if (approvals?.length) {
            ...
            stakingData = {
                ...stakingData,
                approvedBalance: BigNumber.from(approvalData.value) ?? DEFAULT_RECEIVER_STAKING_DATA.approvedBalance
            };
        }
        return stakingData;
    }
...
export async function checkIfSignerExistsInSubgraph(address) {
    const url = ENVIRONMENT.public_contract_subgraph_url;
...
export async function getPondAndMPondBridgeAllowances(address, contractAddress) {
    const url = ENVIRONMENT.public_contract_subgraph_url;
...
export async function getRequestedMPondForConversion(address) {
...
export async function getPondToMPondConversionHistory(address) {
    const url = ENVIRONMENT.public_bridge_contract_subgraph_url;
    ...
    try {
        ...
        const pondToMpondConversions = user?.pondToMpondConversions?.map((conversion) => {
            return {
...
export async function getMPondToPondConversionHistory(address) {
    const url = ENVIRONMENT.public_bridge_contract_subgraph_url;
    ...
    try {
        const result = await fetchHttpData(url, options);
        if (!result['data']?.users?.length || !result['data']?.states?.length)
            return undefined;
        const user = result['data']['users'][0];
...
//# sourceMappingURL=subgraphController.js.map