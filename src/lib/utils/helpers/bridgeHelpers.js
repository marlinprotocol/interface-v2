...
export const getModifiedMPondToPondHistory = (mpondToPondConversions, requests, state) => {
    ...
    const ret = requests.map((request) => {
        const { id, mpondAmount, mpondConverted, releaseTime, requestEpoch, timestamp, transactionHash, isCancelled, cancelHash } = request;
        //basic conversions
        ...
        const _firstCycleStartTime = _releaseStartTime < _liquidityStartTime ? _liquidityStartTime : _releaseStartTime;
...
export const bridgeTxnUrls = (txnHash) => {
