...
export async function setSignerAddress(address) {
    ...
    const receiverStakingContract = new ethers.Contract(receiverStakingContractAddress, receiverStakingContractAbi, signer);
    try {
        addToast({
        ...
        addToast({
            message: MESSAGES.TOAST.TRANSACTION.SUCCESS +
                ' ' +
...
export async function depositStakingToken(amount, signerAddress = '') {
    const receiverStakingContractAddress = contractAddresses.ReceiverStaking;
    const receiverStakingContractAbi = contractAbi.ReceiverStaking;
    const receiverStakingContract = new ethers.Contract(receiverStakingContractAddress, receiverStakingContractAbi, signer);
    try {
        ...
        addToast({
            message: MESSAGES.TOAST.TRANSACTION.SUCCESS +
                ' ' +
                MESSAGES.TOAST.ACTIONS.DEPOSIT.POND_DEPOSITED(bigNumberToCommaString(amount, pondPrecisions)),
            variant: 'success'
        });
    }
    catch (error) {
...
export async function withdrawStakingToken(amount) {
    const receiverStakingContractAddress = contractAddresses.ReceiverStaking;
    const receiverStakingContractAbi = contractAbi.ReceiverStaking;
    const receiverStakingContract = new ethers.Contract(receiverStakingContractAddress, receiverStakingContractAbi, signer);
    try {
        ...
        addToast({
            message: MESSAGES.TOAST.TRANSACTION.SUCCESS +
                ' ' +
                MESSAGES.TOAST.ACTIONS.WITHDRAW.POND_WITHDREW(bigNumberToCommaString(amount, pondPrecisions)),
            variant: 'success'
        });
        return tx;
    }
...
export async function approvePondTokenForReceiverStaking(amount) {
    const receiverStakingContractAddress = contractAddresses.ReceiverStaking;
    ...
    try {
        ...
        addToast({
            message: MESSAGES.TOAST.TRANSACTION.SUCCESS +
                ' ' +
                MESSAGES.TOAST.ACTIONS.APPROVE.POND_APPROVED(bigNumberToCommaString(amount, pondPrecisions)),
            variant: 'success'
        });
        return tx;
    }
...
export async function approvePondTokenForConversion(amount) {
...
export async function approveMPondTokenForConversion(amount) {
    const bridgeContractAddress = contractAddresses.Bridge;
    const mPondTokenContractAddress = contractAddresses.tokens['MPOND'].address;
    const ERC20ContractAbi = contractAbi.ERC20;
    const mPondTokenContract = new ethers.Contract(mPondTokenContractAddress, ERC20ContractAbi, signer);
    try {
        addToast({
            message: MESSAGES.TOAST.ACTIONS.APPROVE.MPOND(bigNumberToCommaString(amount, mPondPrecisions)),
            variant: 'info'
        });
        ...
        addToast({
            message: MESSAGES.TOAST.TRANSACTION.SUCCESS +
                ' ' +
                MESSAGES.TOAST.ACTIONS.APPROVE.MPOND_APPROVED(bigNumberToCommaString(amount, mPondPrecisions)),
            variant: 'success'
        });
        return tx;
    }
...
export async function convertPondToMPond(expectedMPond) {
    const bridgeContractAddress = contractAddresses.Bridge;
    ...
    try {
        addToast({
            message: MESSAGES.TOAST.ACTIONS.CONVERT.POND_TO_MPOND_CONVERTING(bigNumberToCommaString(expectedMPond, mPondPrecisions)),
            variant: 'info'
        });
        ...
        addToast({
            message: MESSAGES.TOAST.TRANSACTION.SUCCESS +
                ' ' +
                MESSAGES.TOAST.ACTIONS.CONVERT.POND_TO_MPOND_CONVERTED(bigNumberToCommaString(expectedMPond, mPondPrecisions)),
            variant: 'success'
        });
        return tx;
    }
...
export async function requestMPondConversion(amount) {
    const bridgeContractAddress = contractAddresses.Bridge;
    const bridgeContractAbi = contractAbi.Bridge;
    const bridgeContract = new ethers.Contract(bridgeContractAddress, bridgeContractAbi, signer);
    try {
        addToast({
            message: MESSAGES.TOAST.ACTIONS.REQUEST.MPOND_TO_POND_REQUESTING(bigNumberToCommaString(amount, mPondPrecisions)),
            variant: 'info'
        });
        ...
        addToast({
            message: MESSAGES.TOAST.TRANSACTION.SUCCESS +
                ' ' +
                MESSAGES.TOAST.ACTIONS.REQUEST.MPOND_TO_POND_REQUESTED(bigNumberToCommaString(amount, mPondPrecisions)),
            variant: 'success'
        });
        return tx;
    }
...
export async function cancelMPondConversionRequest(epoch) {
    const bridgeContractAddress = contractAddresses.Bridge;
    const bridgeContractAbi = contractAbi.Bridge;
    const bridgeContract = new ethers.Contract(bridgeContractAddress, bridgeContractAbi, signer);
    try {
...
export async function confirmMPondConversion(epoch, amount) {
    const bridgeContractAddress = contractAddresses.Bridge;
    const bridgeContractAbi = contractAbi.Bridge;
    const bridgeContract = new ethers.Contract(bridgeContractAddress, bridgeContractAbi, signer);
    try {
        addToast({
            message: MESSAGES.TOAST.ACTIONS.CONVERT.MPOND_TO_POND_CONVERTING(bigNumberToCommaString(amount, mPondPrecisions)),
            variant: 'info'
        });
        ...
        addToast({
            message: MESSAGES.TOAST.TRANSACTION.SUCCESS +
                ' ' +
                MESSAGES.TOAST.ACTIONS.CONVERT.MPOND_TO_POND_CONVERTED(bigNumberToCommaString(amount, mPondPrecisions)),
            variant: 'success'
        });
        return tx;
    }
