...
import { extractBalanceColumnData, extractHourlyRateColumnData, extractRegionColumnData, isSortedNumerically } from '../../helpers/marketplace';
...
test('Withdraw funds more than available balance should not be allowed', async ({ context, page, metamaskPage, extensionId }) => {
    await page.goto(OYSTER_OWNER_INVENTORY_URL, { waitUntil: 'networkidle' });
    ...
    if (expandRowToggleButton && !(await expandRowToggleButton.isDisabled())) {
        ...
        await page.locator('form').filter({ hasText: `Amount MAX Available balance:` }).first().getByPlaceholder('0.00').fill(`${maxAvailableBalance + 1}`);
        await page.waitForSelector('text=Insufficient balance');
    }

})

test('Withdraw funds less than available balance should be allowed', async ({ context, page, metamaskPage, extensionId }) => {
    await page.goto(OYSTER_OWNER_INVENTORY_URL, { waitUntil: 'networkidle' });
    ...
    const durations = durationStrings.map((timeString) => {
    ...
    if (expandRowToggleButton && !(await expandRowToggleButton.isDisabled())) {
        ...
        await page.locator('text=WITHDRAW').first().click();
        const availableBalanceText = await page.locator('form').filter({ hasText: `Amount MAX Available balance:` }).first().innerText();
        ...
        if (availableAmount === 0.00 || availableAmount === 0) {
            console.log('no amount to withdraw.')
            test.skip();
        }

        await page.waitForSelector(`text=Enter the amount you'd like to withdraw from this job.`);
        await page.locator('form').filter({ hasText: `Amount MAX Available balance:` }).first().getByPlaceholder('0.00').fill(`${findSmallerNumber(availableAmount)}`);
        ...
        await metamask.confirmTransactionAndWaitForMining();
        await page.waitForSelector(`text=${MESSAGES.TOAST.TRANSACTION.SUCCESS} ${MESSAGES.TOAST.ACTIONS.WITHDRAW_JOB.WITHDRAWN}`)
    }
