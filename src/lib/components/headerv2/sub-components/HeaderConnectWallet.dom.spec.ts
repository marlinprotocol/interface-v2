...
describe('HeaderConnectWallet', () => {
	it('renders the Header Links Group component properly', () => {
		const { container } = render(HeaderConnectWallet);
		expect(container).toMatchSnapshot();
	});

	it('renders ConnectWalletButton if wallet is not connected', () => {
	...
	it('renders DisconnectWalletButton if wallet is not connected', () => {
	...
	it('toggles between connected and disconnect state and render proper components', async () => {
		connected.mockSetSubscribeValue(true);
