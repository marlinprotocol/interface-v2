 ...
 */
export const epochToDurationString = (epoch, mini = false) => {
...
function roundNumberString(numString, decimals = 2) {
 ...
 * @returns string
 */
export const bigNumberToCommaString = (value, decimals = 2) => {
 ...
 * @returns string
 */
export const bigNumberToString = (value, bigNumberDecimal = 18) => {
    if (!value)
        return '0.00';
...
export const stringToBigNumber = (value, bigNumberDecimal = 18) => {
    if (!value)
...
export const epochSecToString = (date) => {
...
export const shortenText = (text, first = 6, last = 4) => {
    if (text.length <= first + last) {
        return text;
    }
    return text.slice(0, first) + '...' + text.slice(-last);
};
export const mPondToPond = (mPond) => {
    //one mPond is 10^6 pond
    return mPond.mul(ethers.BigNumber.from(10).pow(6));
};
export const pondToMPond = (pond) => {
